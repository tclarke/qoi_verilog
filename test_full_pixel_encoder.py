...
async def recv(dut):
    ...
    cnt = 0
    await ReadOnly()
    while dut.wr_en.value == 1:
        pixvalue = (pixvalue << 8) | dut.ostream.value
        dut._log.debug("Got byte %x (%x) <--> %d", dut.ostream.value, pixvalue, dut.state.value)
        cnt += 1
...
@cocotb.test()
async def test_full_pixel_endoder(dut):
    """
    Test the full pixel encoder.
    """
    # Create a clock
